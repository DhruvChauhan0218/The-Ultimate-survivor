...
public class Astar
{
    ...
    public List<Spot> CreatePath(Vector3Int[,] grid, Vector2Int start, Vector2Int end, int length, int reachableHeight)
    {
        ...
        if (!IsValidPath(Start, End, reachableHeight))
            return null;

        Debug.Log("IsVaildPath");
        ...
        while (OpenSet.Count > 0)
        {
            ...
            // Height != 0
            // Height = currentSpotHeight or Node = ladder or currentNode = ladder
            // Height < ReachableHeight
            // Finds the next closest step on the grid
            // If player is on ladder then can not move left or right
            //Debug.Log("neighboors: " + current.Neighboors.Count);
            int currentSpotHeight = current.Height;
            ...
            for (int i = 0; i < neighboors.Count; i++)//look threw our current spots neighboors (current spot is the shortest F distance in openSet
            {
                ...
                if (!ClosedSet.Contains(n) && n.Height <= reachableHeight && ( n.Height == currentSpotHeight)) //Checks to make sure the neighboor of our current tile is not within closed set, and has a height of greater than 1
                {
                    //Debug.Log("ClosedSet");
    ...
    private bool IsValidPath(Spot start, Spot end, int reachableHeight)
    {
        if (end == null)
            return false;
        if (start == null)
        ...
        //Debug.Log("end.Height: " + end.Height);
        //Debug.Log("reachableHeight: " + reachableHeight);

        if (end.Height > reachableHeight)
            return false;
        return true;
    }
